...
module.exports = function(data) {
    ...
    let items = (input[5] === 'empty'
        ? []
        : input[5].split(' ').map((e) => parseInt(e)).reduce((foods, val, i) => {
            if (!(i % 3)) {
    ...
    ).concat(input[6] === 'empty'
        ? []
        : input[6].split(' ').map((e) => parseInt(e)).reduce((coins, val, i) => {
            if (!(i % 3)) {
    ...
    if (startHealth < 40) {
        for (let i = 1; i < items.length; i++) {
            if (items[i][0] === 'food' && items[i][3] > best[0][3])
                best[0] = items[i];
        }
    } else {
        for (let i = 1; i < items.length; i++) {
            if (items[i][0] === 'coin' && items[i][3] > best[0][3])
                best[0] = items[i];
        }
    }
    ...
    if (best[0][0] === 'food') {
        health = Math.min(best[0][3], 100);
    } else {
        coins += best[0][3];
    }

    return best[0][1] + ' ' + best[0][2] + ' ' + health + ' ' + coins + ' ' + direction;
}
